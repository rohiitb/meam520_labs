...
def pick_static_block(team, start_position, arm, detector, ik, fk, subsequent_scanning=False):
    blk_cnt = 0
    ...
    if team == 'red':
        ...
        start_position_2 = np.array([-0.01779206-pi/9, -0.760123540+.610865,  0.01978261, -2.34205014+pi/8, 0.02984053, 1.54119353+pi/11, 0.75344866]) # Can change this to b emore closer to the blocks
    ...
    if subsequent_scanning == True:
        for i in range(4):
            ...
            if len(detections_list) == 0:
                arm.safe_move_to_position(start_position_2)
...
def dynamic_block(team, arm):
    arm.open_gripper()
    if team == 'red':
        start_position_dyn = np.array([0.80563,  1.31788 , 0.79382, -0.88222,  0.68683 , 1.44763 ,-1.16219])    # Final seed for trying
        # start_position_dyn = np.array([   0.8277,   1.38496 , 0.79852 ,-0.74768 , 0.66156 , 1.38145 ,-1.12892]) # FFFFFFFFFFFFFFFFFFF
        # start_position_dyn_0 = np.array([  -0.01779206-pi/9, -0.76012354+pi/6,  0.01978261, -2.34205014+pi/8, 0.02984053, 1.54119353+pi/11, 0.75344866])

        # arm.safe_move_to_position(start_position_dyn_0)

        arm.safe_move_to_position(start_position_dyn)
    else:
        # start_position_dyn = np.array([0.8104 , -0.89688, -2.15937, -1.34125, -0.74377 , 1.85504 , 2.60007])    # Final seed for trying
        start_position_dyn = np.array([0.66292, -1.53987, -1.86795, -0.91314,  0.20498,  1.5493, 2.14179])
        # start_position_dyn = np.array([   0.8277,   1.38496 , 0.79852 ,-0.74768 , 0.66156 , 1.38145 ,-1.12892]) # FFFFFFFFFFFFFFFFFFF
        # start_position_dyn_0 = np.array([  -0.01779206+pi/9, -0.76012354+pi/6,  0.01978261, -2.34205014+pi/8, 0.02984053, 1.54119353+pi/11, 0.75344866])
...
if __name__ == "__main__":
        ...
        # pick_static_block(team, start_position, arm, detector, ik, fk, subsequent_scanning=True)

        # Dynamic block 
        dynamic_block(team, arm)
